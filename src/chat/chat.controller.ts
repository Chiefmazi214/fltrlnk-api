...
export class ChatController {
  ...
  @ApiResponse({
    status: 403,
    description: 'Forbidden - User not in chat room',
  })
  async getChatRoomMessages(
    ...
    const canAccess = await this.chatService.canUserJoinRoom(
      req.user._id,
      chatRoomId,
    );
    if (!canAccess) {
  ...
  async createChatRoom(
    ...
    const existingChatRoom = await this.chatService.findChatRoomByUsers(
  ...
  @ApiResponse({
    status: 200,
    description: 'Chat room type switched successfully',
  })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({ status: 400, description: 'Bad Request - Invalid input' })
